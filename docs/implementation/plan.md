...
## ADR-1: 2026-07-20 — Cross-Game Player Profile (Meta-Progression Layer)

### Context

`mobile-gaming` ships 13 fully self-contained games behind a hub (`src/hub/`) — each game is independently playable at its own URL with zero dependency on the hub or other games, and each game's progress lives in its own `shared/storage.js` namespace. This isolation is deliberate (see "Objective" above) and correctly keeps individual games simple and portable.

The consequence, verified against the live deploy at `https://mobile-gaming.pages.dev/`, is that nothing rewards a player for engaging with the *catalog* — only with a single game. A player who masters Water Sort has zero in-product reason to try Bus Jam or Crowd Runner; the hub is a menu, not a loop. This matters because the collection's core distribution advantage is breadth (13 games under one roof, one PWA install, one daily-challenge cadence via `shared/daily.js`) — an advantage the product currently does nothing to monetize in engagement terms.

At the same time, a large concurrent body of in-progress work (16 in-progress beads as of 2026-07-20, e.g. `bf-lat2w` retry overlay, `bf-5ucet` level-nav, `bf-2ivj9` audio, `bf-1huma` daily challenge, `bf-4yc9o` shareable state URLs) is wiring *already-built* shared modules into the 13 games' win/level-complete call sites. Those beads establish exactly the integration points (`onLevelComplete`, `onGameWin`, retry/level-nav UI regions) that any new cross-game system would also need to hook — so a new meta-progression layer is cheapest to land once, not before, those land.

### Decision

Introduce a client-side, server-free **Player Profile** layer:

- `shared/profile.js` — a single global XP + coin ledger, namespaced in `localStorage` like every other shared module (`storage.js` pattern). Every `level_complete` analytics event (already emitted uniformly by all 13 games per the Observability section below) awards XP and coins; no per-game special-casing required.
- `shared/achievements.js` — cross-game milestones ("play 3 different games in one day", "complete 25 levels total across the catalog", "finish a daily challenge 5 days in a row") surfaced as toast notifications, reusing the existing toast/overlay patterns from `shared/retry.js`.
- A hub-level profile header (level badge + coin count) and a new "Rewards" tab where coins unlock **cosmetic-only** rewards — alternate palettes via the existing `shared/colors.js` / `shared/color-blind.js` theming system. No pay-to-win, no gameplay-affecting unlocks.

This is additive: it does not touch `state.js` or `generator.js` in any game (both are documented as pure and never modified), and it rides on the analytics event stream that already exists.

### Alternatives Considered

1. **Status quo — per-game progression only.** Simplest, zero new code. Rejected: leaves the 13-game catalog effect completely unexploited; the daily-challenge and streak infrastructure that already exists is designed to pull players back, but nothing pulls them *across* games once they're back.
2. **Server-backed global leaderboards/accounts** (Cloudflare Workers + KV or D1). Gives a "real" competitive multiplayer feel and enables cross-device sync. Rejected for now: it breaks the deliberate zero-server, no-PII, static-hosting architecture (see Tech Stack and Client-Side Analytics sections — "No server, no PII, no cookies, no third-party scripts"), adds auth, moderation, and cheating-surface concerns disproportionate to a hyper-casual collection, and duplicates infrastructure work rather than product work. Worth revisiting later as a layer on top of the local profile, once engagement lift is proven — `shared/sync.js` already exists as a wired-nowhere stub (`bf-4se8v`) that a future device-sync effort could extend.
3. **Full unlock/IAP economy with monetization gates.** Out of scope: no monetization infrastructure exists in this repo, and adding one is a distinct product decision, not a natural extension of "improve what's shipped."
4. **Achievements only, no currency.** Smaller surface, but weaker delight loop — a checklist with no spendable reward. Rejected in favor of the currency + cosmetic-unlock loop, which gives players something to *do* with progress, not just observe it.

### Consequences

- **Positive:** every completed level in any of the 13 games feeds one meter, giving players an explicit reason to explore games they haven't tried — the single biggest lever this repo has for engagement that doesn't require building a 14th game.
- **Positive:** zero new infrastructure — stays entirely within the existing "no server, no PII" static-hosting model and reuses `storage.js`, `analytics.js`'s `level_complete` event, and `colors.js` theming rather than inventing new plumbing.
- **Negative:** `shared/profile.js` and the hub UI still need to be wired into all 13 games' win-flow call sites — the same integration burden the current in-progress beads (`bf-lat2w`, `bf-5ucet`, `bf-2ivj9`, etc.) are already paying down. This ADR should be sequenced *after* those land, and the implementation should piggyback on the same call sites rather than opening a second wave of per-game edits.
- **Negative:** localStorage-only means profile progress does not sync across devices — acceptable short-term given `sync.js` is already a known no-op (`bf-4se8v`); flagged as a future extension point, not a blocker.
- **Negative:** unbounded ledger growth risk — mitigate with the same LRU-capped append pattern `analytics.js` already uses for its 500-entry event log, rather than an unbounded array.
