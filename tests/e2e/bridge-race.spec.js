/**
 * Bridge Race - E2E Tests (Playwright)
 */

import { test, expect } from '@playwright/test';

const GAME_URL = '/bridge-race/';
...
test.describe('Bridge Race', () => {
  ...
  test('loads game page with correct title', async ({ page }) => {
    await expect(page).toHaveTitle(/Bridge Race/);
    await expect(page.locator('h1')).toContainText('Bridge Race');
  });
  ...
  test('displays initial stats on level 1', async ({ page }) => {
    ...
    await expect(page.locator('#block-count')).toContainText('0');
