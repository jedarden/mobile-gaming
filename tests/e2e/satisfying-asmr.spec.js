/**
 * Satisfying Clean (Satisfying ASMR) - E2E Tests (Playwright)
 */

import { test, expect } from '@playwright/test';

const GAME_URL = '/satisfying-asmr/';
...
test.describe('Satisfying Clean', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const gameModulePromise = page.waitForResponse(response =>
      response.url().includes('/src/games/satisfying-asmr/game.js') && response.status() === 200
    );
  ...
  test('loads game page with correct title', async ({ page }) => {
    await expect(page).toHaveTitle(/Satisfying/i);
  });
  ...
  test('displays initial stats on level 1', async ({ page }) => {
    ...
    await expect(page.locator('#progress-display')).toContainText('0%');
