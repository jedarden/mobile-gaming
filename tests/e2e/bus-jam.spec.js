/**
 * Bus Jam - E2E Tests (Playwright)
 */

import { test, expect } from '@playwright/test';

const GAME_URL = '/bus-jam/';
...
test.describe('Bus Jam', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const gameModulePromise = page.waitForResponse(response =>
      response.url().includes('/src/games/bus-jam/game.js') && response.status() === 200
    );
  ...
  test('loads game page with correct title', async ({ page }) => {
    await expect(page).toHaveTitle(/Bus Jam/i);
  });
  ...
  test('displays initial stats on level 1', async ({ page }) => {
    ...
    await expect(page.locator('#passengers-display')).toBeVisible();
