/**
 * Crowd Runner - E2E Tests (Playwright)
 */

import { test, expect } from '@playwright/test';

const GAME_URL = '/crowd-runner/';
...
test.describe('Crowd Runner', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const gameModulePromise = page.waitForResponse(response =>
      response.url().includes('/src/games/crowd-runner/game.js') && response.status() === 200
    );
  ...
  test('loads game page with correct title', async ({ page }) => {
    await expect(page).toHaveTitle(/Crowd Runner/);
    await expect(page.locator('h1')).toContainText('Crowd Runner');
  });
